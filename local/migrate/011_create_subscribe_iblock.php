<?php
/**
 * Создает инфоблок «Подписка на рассылку»
 *
 ...
 * Создает инфоблок «Подписка на рассылку»
 *
 * Class CreateSubscribeIBlockMigration
 */
class CreateSubscribeIBlockMigration extends AbstractIBlockMigration
{
    /**
     * {@inheritdoc}
     */
    public function up()
    {
        ...
        try {
            ...
            $logger->log(
                sprintf('IBlock has been created. Id: "%s". Add to "subscribeIBlockId"', $this->iblockId)
            );
    ...
    public function down()
    {
        ...
        $this->deleteIBlock($environment->get('subscribeIBlockId'));

        $logger->log(sprintf('IBlock rewards has been removed. Id: "%s"', $this->iblockId));
    }
}

$migration = new CreateSubscribeIBlockMigration();
$migration->up();
?>
