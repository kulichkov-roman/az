...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	if(COption::GetOptionString('main', 'use_encrypted_auth', 'N') == 'Y')
	{
		...
		if(($arKeys = $sec->LoadKeys()))
		{
			...
			if($errno == CRsaSecurity::ERROR_SESS_CHECK)
				$arResult["ERRORS"]["main_register_sess_expired"] = GetMessage("main_register_sess_expired");
			elseif($errno < 0)
				$arResult["ERRORS"]["main_register_decode_err"] = GetMessage("main_register_decode_err", array("#ERRCODE#"=>$errno));
		}
	}
	...
	{       
        // Устанавливаем e-mail в качестве логина        
        if($key == 'LOGIN' && empty($arResult["VALUES"][$key])){
            $arEmail = explode('@', $_REQUEST["REGISTER"]['EMAIL']);
            
            $login = is_array($arEmail) ? $_REQUEST["REGISTER"]['EMAIL'] : $arEmail[0];
            $rsUser = CUser::GetByLogin($login);
            if($arUser = $rsUser->Fetch()){
                $login .= '1';
            }
            
            $arResult["VALUES"][$key] = $login;
	...
	if ($arResult["USE_CAPTCHA"] == "Y")
	{
		if (!$APPLICATION->CaptchaCheckCode($_REQUEST["captcha_word"], $_REQUEST["captcha_sid"]))
			$arResult["ERRORS"]["REGISTER_WRONG_CAPTCHA"] = GetMessage("REGISTER_WRONG_CAPTCHA");
	}
	...
	if(strlen($arResult["VALUES"]["EMAIL"]) > 0 && COption::GetOptionString("main", "new_user_email_uniq_check", "N") === "Y")
	{
		...
		if($res->Fetch())
			$arResult["ERRORS"]["REGISTER_USER_WITH_EMAIL_EXIST"] = GetMessage("REGISTER_USER_WITH_EMAIL_EXIST", array("#EMAIL#" => htmlspecialcharsbx($arResult["VALUES"]["EMAIL"])));
	}
	...
	{
        // Положим в сессию флаг ошибочной регистрации, что-бы на странице вызвать скрипт всплывающего окна регистрации
        $_SESSION['MB_SUCCESS_REGISTRATION'] = false;
	...
	{
        // Положим в сессию флаг ошибочной регистрации, что-бы на странице вызвать скрипт всплывающего окна регистрации
        $_SESSION['MB_SUCCESS_REGISTRATION'] = true;
		...
		if (intval($ID) > 0)
		{
            ...
            // Положим в сессию флаг успешной регистрации, что бы на странице вызвать скрипт всплывающего окна сообщения об успешной регистрации
            $_SESSION['MB_SUCCESS_REGISTRATION'] = true;                
		}
		else
		{
