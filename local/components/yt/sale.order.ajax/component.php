...
$arProfileName = array();
$strProfileName = '';
$arProfileNameCodes = array("STREET", "FLAT", "HOUSE");
...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if($isOrderPlaced)
		{
			...
			while ($arOrderProps = $dbOrderProps->GetNext())
			{
				...
				//{
					...
					if(in_array($arOrderProps["CODE"], $arProfileNameCodes)){
						$arProfileName[] = $curVal;
					}
					...
					elseif ($arOrderProps["IS_PROFILE_NAME"]=="Y" || $arOrderProps["IS_PAYER"]=="Y" || $arOrderProps["IS_EMAIL"]=="Y" || $arOrderProps["IS_ZIP"]=="Y")
					{
						...
						{
							if(!$USER->IsAuthorized()){
								$arUserResult["USER_EMAIL"] = "NOT_REGISTER@example.ru";
							}else{
		...
		if($GLOBALS["USER"] -> IsAuthorized()){
			...
			$userPrVal = 1;
			if($arUser["UF_TYPE_PROFILE"] == 14){$userPrVal = 2;}elseif($arUser["UF_TYPE_PROFILE"] == 15){$userPrVal = 3;}
		} 
		/* Person Type Begin */
		if(isset($userPrVal) && (!isset($arUserResult["PERSON_TYPE_ID"]) || IntVal($arUserResult["PERSON_TYPE_ID"]) <= 0)){
			$dbPersonType = CSalePersonType::GetList(array("SORT" => "ASC", "NAME" => "ASC"), array("LID" => SITE_ID, "ACTIVE" => "Y"));
			while($arPersonType = $dbPersonType->GetNext())
			{
				
				if($userPrVal == $arPersonType["ID"])
				{
					$arUserResult["PERSON_TYPE_ID"] = $arPersonType["ID"];
					$arPersonType["CHECKED"] = "Y";
				}
		...
		foreach($arResult["ORDER_PROP"]["USER_PROFILES"] as $key => $arProfile){
			if(!isset($_POST["PROFILE_ID"])){
				if($arProfile["ID"] == $defaultProfileId){
					...
					$arUserResult["PROFILE_ID"] = IntVal($arProfile["ID"]);
				} else {
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			...
			{		
				...
				if (empty($arResult["ERROR"]))
				{
					CSaleOrderUserProps::DoSaveUserProfile($USER->GetID(), $arUserResult["PROFILE_ID"], implode(", ", $arProfileName) /*$arUserResult["PROFILE_NAME"]*/, $arUserResult["PERSON_TYPE_ID"], $arUserResult["ORDER_PROP"], $arResult["ERROR"]);
				}
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if (!empty($arBasketList) && is_array($arBasketList))
					{
						foreach ($arBasketList as &$arBasketItem)
						{
							$arBasketItem['PROPERTIES'] = array(
								'TIPTOVARA' => array(
									'VALUE' => $arElements[$arBasketItem['PRODUCT_ID']]['PROPERTY_TIPTOVARA_VALUE']
								)
							);
						}
					}
					...
					$arMeasureCurrency = $GLOBALS["measureCurrency"];
					...
					if (!empty($arBasketList) && is_array($arBasketList))
					{
						foreach ($arBasketList as &$arBasketItem)
						{
							...
							{
								if($type == $arBasketItem['PROPERTIES']['TIPTOVARA']['VALUE'])
								{
									$typeElement = $type;
									$arBasketItem['PROPERTIES']['TIPTOVARA']['TYPE'] = $value;
									break;
								}
							}
							 ...
							 */
							$arBasketItem['PROPERTIES']['TIPTOVARA']['MEASURE'] = $arMeasureElement[$arBasketItem['PROPERTIES']['TIPTOVARA']['TYPE']];
							$arBasketItem['PROPERTIES']['TIPTOVARA']['MEASURE_PRICE'] = $arMeasurePriceElement[$arBasketItem['PROPERTIES']['TIPTOVARA']['TYPE']];

							/**
							 * Определить количество грамм
							 */
							$arBasketItem['PROPERTIES']['TIPTOVARA']['STEP'] = $arStepElement[$typeElement];

							unset($typeElement);

							$measureText = $arBasketItem['PROPERTIES']['TIPTOVARA']['MEASURE'];
							$quantity = $arBasketItem["QUANTITY"] * $arBasketItem['PROPERTIES']['TIPTOVARA']['STEP'];
							$price = getPrintFormatPrice($arBasketItem["QUANTITY"] * $arBasketItem['PRICE'], '', $class = '', 'wh_measure_mail_tmp');

							$strOrderList .= $arBasketItem["NAME"]." - ".$quantity." ".$measureText.": ".$price;
							$strOrderList .= "\n";
						}
					...
					$arFields = array(
						...
						/**
						 * Разделять в почтовом уведомлеии цену заказа и цену доставки
						 */
						"PRICE" => SaleFormatCurrency($orderTotalSum - $arResult["DELIVERY_PRICE"], $arResult["BASE_LANG_CURRENCY"]),
			...
			elseif($NOT_REGISTER_USER > 0 && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
					/*CSaleOrderUserProps::DoSaveUserProfile($NOT_REGISTER_USER, $arUserResult["PROFILE_ID"], implode(", ", $arProfileName), $arUserResult["PERSON_TYPE_ID"], $arUserResult["ORDER_PROP"], $arResult["ERROR"]);*/
				}
				...
				if (empty($arResult["ERROR"]))
				{
					...
					{
						$event = new CEvent;
						$event->Send("SALE_NEW_ORDER_NOT_REGISTR", SITE_ID, $arFields, "N");
					}
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if(array_key_exists('json', $_REQUEST) && $_REQUEST['json'] == "Y" && ($arParams["ALLOW_AUTO_REGISTER"] == "Y"))
					{
						if($arUserResult["CONFIRM_ORDER"] == "Y" || $arResult["NEED_REDIRECT"] == "Y")
						{
							$APPLICATION->RestartBuffer();
	...
	else
	{
		...
		if ($arOrder && ($arOrder["USER_ID"] == IntVal($USER->GetID()) || $arOrder["USER_ID"] == 690))
		{
