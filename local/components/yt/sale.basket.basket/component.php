...
if (strlen($_REQUEST["BasketRefresh"]) > 0 || strlen($_REQUEST["BasketOrder"]) > 0 || strlen($_REQUEST[$arParams["ACTION_VARIABLE"]]) > 0 )
{
	...
	{
		if ($_REQUEST["BasketOrderVal"] == 1){		// for coupon	
		...
		//if (!empty($_REQUEST["BasketOrder"]) && empty($arResult["WARNING_MESSAGE"])) for coupon
		if ($_REQUEST["BasketOrderVal"] == 0 && !empty($_REQUEST["BasketOrder"]) && empty($arResult["WARNING_MESSAGE"]))
		{
